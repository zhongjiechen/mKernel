    ...
    void post_remote_flag_batch(ibv_qp_ex* qpx, uint32_t dst_qpn,
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (!cfg_.use_arrival_queue) {
                ...
                // The payload (cfg_.epoch) is constant across the whole epoch
                // and was written into host_ptr[0] once at the top of this
                // function, so all in-flight WRs may safely share the same
                // SGE source slot.
                qpx->wr_id = 1;
                qpx->comp_mask = 0;
                qpx->wr_flags = IBV_SEND_SIGNALED;
