...
struct KittensBroker {
    // Keys can be overridden via env KITTENS_BROKER_KEY to avoid socket/shm conflicts between runs
    ...
    __host__ inline KittensBroker(int local_rank, int local_world_size)
        ...
        const char *base = std::getenv("KITTENS_BROKER_KEY");
        if (base && base[0] != '\0') {
            (void)snprintf(shm_key_, KEY_MAX, "/%s_shm", base);
            (void)snprintf(sock_key_, KEY_MAX, "/tmp/%s.sock", base);
        } else {
            (void)snprintf(shm_key_, KEY_MAX, "/kittens_broker_shm");
            (void)snprintf(sock_key_, KEY_MAX, "/tmp/kittens_broker.sock");
        }
