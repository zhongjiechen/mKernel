...
#include "../common/tk_types_system_vmm.cuh"
#include "../common/tk_types_system_ipc.cuh"
#include "broker.cuh"
...
struct TKParallelTensor {
    ...
    std::optional<detail::vmm::handle> local_vmm_handle_;
    ...
    __host__ inline void initialize_multicast() {
        ...
        const bool bind_retained_handle = []() {
            const char *v = std::getenv("OSGC_BIND_RETAINED_HANDLE");
            return v && std::string(v) == "1";
        }();
        if (bind_retained_handle) {
            CUmemGenericAllocationHandle retained_handle;
            detail::vmm::vm_retrieve_handle(&retained_handle, reinterpret_cast<void *>(data_.data_ptr()));
            detail::vmm::multicast_bind_memory(multicast_handle, retained_handle, allocated_size_);
            detail::vmm::vm_free(retained_handle);
        } else {
            detail::vmm::multicast_bind_memory(multicast_handle, *local_vmm_handle_, allocated_size_);
        }
        brokers_.at({local_rank_, local_world_size_}).sync();

        // Map at the same virtual address on all processes so multimem red/ld_reduce see the same slot (CUDA requirement).
        ...
        if (local_rank_ != 0) {
            detail::vmm::vm_map_at(&multicast_ptr_, reinterpret_cast<void *>(mc_addrs[0]), multicast_handle, multicast_allocated_size_);
        }
    ...
    __host__ inline void destroy() {
        ...
        // 3.5 Release local VMM allocation handle (if any). The backing allocation
        // remains alive until all mappings are unmapped.
        if (local_vmm_handle_.has_value()) {
            detail::vmm::vm_free(*local_vmm_handle_);
...
#define BIND_TK_PARALLEL_TENSOR(m) \
    pybind11::class_<kittens::py::TKParallelTensor>(m, "TKParallelTensor") \
        ...
        .def_property_readonly("multicast_ptr_u64", [](const kittens::py::TKParallelTensor &t) { \
            return static_cast<uint64_t>(reinterpret_cast<uintptr_t>(t.multicast_ptr_)); \
        }) \
        .def_property_readonly("multicast_enabled", [](const kittens::py::TKParallelTensor &t) { \
            return t.multicast_; \
        }) \
