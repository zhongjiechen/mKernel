#!/usr/bin/env python3
"""Render per-kernel EFA TFLOPS bar charts.

Reads bench/results/<topic>_{efa,nccl}.json and bench/cx7_reference.json,
emits plots/<topic>_efa.png plus a 5-panel overview chart.

The default chart series are NCCL, mKernel, and the CX7 reference line.
"""
...
CX7_PATH = ROOT / "bench" / "cx7_reference.json"
# External baselines for the ring_attention chart only — Mercury + MagiAttention
# results are optional external inputs rather than files generated by this repo.
REPO = ROOT.parent
...
# Optional Triton-distributed data files use legacy short names. Map those
# filenames to the public kernel names.
TRITON_DIST_KEY = {
    "ag_gemm":        "q1",
    # gemm_ar intentionally omitted: TritonDist has no inter-node
    # GEMM+AllReduce kernel (`create_gemm_ar_context` asserts world ==
    # local_world). The available values are NCCL-equivalent
    # placeholders and would duplicate the NCCL bar with a different color.
    "dispatch_gemm":  "q3",
    "gemm_rs":        "q5",
    # ring_attention has no Triton-dist baseline.
}
